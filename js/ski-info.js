...
function updateSkiInfo(data) {
  ...
  slopes.forEach(function (slope) {
    updateSlopeInfo(slope.name.toLowerCase(), slope.status_code);
  });
...
function updateSlopeInfo(slope_name, status) {
  const classname = removeDiacritics(
    slope_name.replace(/\s/g, "").toLowerCase()
  );

  var obj = $("." + classname + " .status");
  console.log("Lift classname: " + classname + " " + status);
  if (status == 2) {
    obj.attr("title", "V provozu");
    obj.addClass("open");
    obj.removeClass("closed");
    ...
    obj.removeClass("open");
    obj.removeClass("closed");
    console.log("Unknown status code: " + classname + " " + status);
  }
}

function updateWeatherInfo(data) {
  $("#temperature").text(
    "temp_0700" in data && $.isNumeric(data.temp_0700) ? data.temp_0700 : "-"
  );
  $("#weathertext").text(
    "weather_0700_text" in data && data.weather_0700_code != 1
      ? data.weather_0700_text
      : ""
  );
  ...
  var snowheight = "0";
  if (
    "snowheight_slopes_max" in data &&
    "snowheight_slopes_min" in data &&
    $.isNumeric(data.snowheight_slopes_min) &&
    $.isNumeric(data.snowheight_slopes_max)
  ) {
    snowheight = data.snowheight_slopes_min + "-" + data.snowheight_slopes_max;
  } else if (
    "snowheight_slopes" in data &&
    $.isNumeric(data.snowheight_slopes)
  ) {
    snowheight = data.snowheight_slopes;
  }
  $("#snowheight").text(snowheight);
  $("#snowtype").text(
    "snowtype_text" in data && data.snowtype_code != 1
      ? data.snowtype_text
      : "žádný"
  );
...
function updateSnowmakingInfo(status) {
  return status
    ? $(".snowmaking").removeClass("d-none")
    : $(".snowmaking").addClass("d-none");
}
